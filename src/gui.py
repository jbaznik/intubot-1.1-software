...
def rounded_rectangle(src, top_left, bottom_right, line_thickness):
    ...
    corner_radius = 20
    # draw straight lines
    cv2.line(src, (p1[0] + corner_radius, p1[1]), (p2[0] - corner_radius, p2[1]), LABEL_GREEN, line_thickness, cv2.LINE_AA)
    cv2.line(src, (p2[0], p2[1] + corner_radius), (p3[0], p3[1] - corner_radius), LABEL_GREEN, line_thickness, cv2.LINE_AA)
    cv2.line(src, (p3[0] - corner_radius, p4[1]), (p4[0] + corner_radius, p3[1]), LABEL_GREEN, line_thickness, cv2.LINE_AA)
    cv2.line(src, (p4[0], p4[1] - corner_radius), (p1[0], p1[1] + corner_radius), LABEL_GREEN, line_thickness, cv2.LINE_AA)

    # draw arcs
    cv2.ellipse(
        src, (p1[0] + corner_radius, p1[1] + corner_radius),
        (corner_radius, corner_radius),
        180.0, 0, 90, LABEL_GREEN, line_thickness, cv2.LINE_AA)
    cv2.ellipse(
        src, (p2[0] - corner_radius, p2[1] + corner_radius),
        (corner_radius, corner_radius),
        270.0, 0, 90, LABEL_GREEN, line_thickness, cv2.LINE_AA)
    cv2.ellipse(
        src, (p3[0] - corner_radius, p3[1] - corner_radius),
        (corner_radius, corner_radius),
        0.0, 0, 90, LABEL_GREEN, line_thickness, cv2.LINE_AA)
    cv2.ellipse(
        src, (p4[0] + corner_radius, p4[1] - corner_radius),
        (corner_radius, corner_radius),
        90.0, 0, 90, LABEL_GREEN, line_thickness, cv2.LINE_AA)
    return src
...
def draw_boxes(frame, predictions):
    ...
    for key, prediction in predictions.items():
        score = predictions[key]["score"]
        # chance line thickness based on score
        if score >= 0.8:
            line_thickness = int(score * 10)
        if  0.5 <= score <= 0.8 :
            line_thickness = int(score * 3)
        if score < 0.5:
            line_thickness = 1
        box = prediction['box']
        xmin = int(box[0])
        ...
        xmax = int(box[2])
        ymax = int(box[3])
        frame_with_annotation = rounded_rectangle(frame_with_annotation, (xmin, ymin), (xmax, ymax), line_thickness)
        frame_with_annotation = cv2.putText(
            frame_with_annotation, key, (xmin + 10, ymin + 30),
            cv2.FONT_HERSHEY_SIMPLEX, 0.9, LABEL_GREEN, 2, cv2.LINE_AA)
    ...
    while True:
        ...
        if active_camera in ['camera1', 'both'] and backend.new_scene and isinstance(backend.frame, np.ndarray):
            ...
            if active_camera == 'both' and isinstance(backend.videohandler_secondary.frame, np.ndarray):
                ...
                numpyframe = cv2.hconcat([numpyframe, resized])
            if horizontal_flip:
...
def data_generator():
    while True:
        ...
        data = {'detections': [],
                'motors': backend.driver.motor_pwm_command,
                'velocities': backend.driver.velocity_vector,
                'network': [{'trained on': backend.computervision.model_name, 'type': 'FasterRCNN mobilenet v3'}],
                'automode': backend.auto_mode,
                'automode_available': backend.driver.auto_mode_available,
                'js_right': backend.serialhandler.joystick_right_button}
...
def key_up():
    ...
    if data['key'] == 'm':
        msg = change_model()
        if backend.computervision.common_objects_running:
            backend.computervision.common_objects_running = False
        else:
            backend.computervision.common_objects_running = True
        return {"message": msg}
    if data['key'] == 'h':
        #msg = horizontal_flip_camera()
        backend.driver.converge_to_home_position()
        #return {"message": msg}
    if data['key'] == 'o':
        backend.driver.move_translational_sled('stop')
    if data['key'] == 'i':
        backend.driver.change_inversion_status()
    #if data['key'] == 'd':
    #    backend.driver.converge_to_home_position()
    if data['key'] == 'Escape':
        print('shut down')
        backend.serialhandler.close()
        time.sleep(0.1)
